...
def test_get_similarity(cache: diskcache.Cache, openai_client: openai.OpenAI):
    ...
    response = emb_model.get_similarity(query, documents, model_settings=ModelSettings(dimensions=512))
    assert response.usage.cache_hits > 0 or response.usage.total_tokens > 0
    assert response.results[0].relevance_score > 0.0
    assert all(response.results[0].relevance_score >= res.relevance_score for res in response.results)


@pytest.mark.asyncio
async def test_get_similarity_async(cache: diskcache.Cache, openai_client_async: openai.AsyncOpenAI):
    ...
    response = await emb_model.get_similarity(query, documents, model_settings=ModelSettings(dimensions=512))
    assert response.usage.cache_hits > 0 or response.usage.total_tokens > 0
    assert response.results[0].relevance_score > 0.0
    assert all(response.results[0].relevance_score >= res.relevance_score for res in response.results)
